...
export default {
    ...
    methods: {
        ...
        getDisciplineByCode(code) {
            return fetch("http://api-sistema-pre-matricula.herokuapp.com/api/curricularComponent/" + code).then(res => res.json());        
        },
        getDisciplinesCodeByEnrollment(enrollment) {
            return fetch("http://api-sistema-pre-matricula.herokuapp.com/api/allocation/student/" + enrollment).then(res => res.json());
        },
